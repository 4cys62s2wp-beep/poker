...
**Der Offline-Betrieb ist seit E-072 nachgewiesen** — mit abgeschaltetem
Server, nicht mit einer Netzemulation. Ursache war `Vary: Origin`: Der Worker
fand seine eigenen Dateien nicht, weil die Seite sie mit `crossorigin`
anfordert und er sie ohne abgelegt hatte. Ein `ignoreVary: true` behebt es;
`serviceworker.test.ts` hält es fest.

**Was noch offen ist,** steht weiter unten unter „Was als Nächstes zu
...
- Sechs Messläufe ohne Beanstandung: `npm run wege` (Erreichbarkeit, 41 eigene
  Bildschirme, keine Sackgasse), `npm run tisch` (Lesbarkeit am Tisch),
  `npm run durchgang` (ein vollständiger Abend, Farbmodi, beide Zustände der
  Startseite, Hand des Tages, Lernpfad, Modul und Lernbildschirm,
  33 Schritte), `npm run pruefen` (Kontrast, Tippflächen,
