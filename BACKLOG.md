...
## Offline-Betrieb unter aktuellem Chromium klären

**Worum es geht.** `npm run ohnenetz` meldet hier 90 von 90 Bildschirmen ohne
Beanstandung — gemessen mit Chromium 141. Auf dem CI-Runner, wo Playwright in
der jeweils neuesten Fassung installiert wird und einen entsprechend neueren
Chromium mitbringt, meldet derselbe Lauf **90 Befunde: jeder Bildschirm null
Zeichen**. Gleicher Quelltext, gleicher Build, anderer Browser.

Dieselbe Richtung zeigt ein zweiter Versuch hier: Wird der Vorschau-Server
wirklich abgeschaltet (statt nur `setOffline` zu setzen), bleibt auch mit
Chromium 141 jeder Bildschirm leer — obwohl der Zwischenspeicher des Service
Workers nachweislich alle 21 gebauten Dateien enthält und ein `fetch()` aus
der Seite heraus sie von dort auch bekommt.

**Warum es wertvoll wäre.** Offline-Betrieb ist eine der drei Zusagen dieser
App (installierbar, offline, ohne Konto). Wenn sie unter aktuellen Browsern
nicht hält, ist das kein Schönheitsfehler, sondern eine falsche Behauptung im
README, in der Beschreibung und im Kopf der Nutzerin, die im Zug sitzt.

**Was daran schwierig ist.** Die Sache ist bisher nicht reproduzierbar
eingekreist:

- `context.setOffline(true)` erreicht die Anfragen des Service Workers nicht
  (gemessen: `fetch(…)` über den Worker liefert 200, während
  `navigator.onLine` false ist). Diese Methode taugt nicht als Nachweis.
- Server abschalten macht die Seite leer, aber der Worker sieht dann gar
  keine Anfragen mehr. Ob das am Browser liegt oder an der Steuerung, ließ
  sich nicht trennen.
- Der Versuch, den Worker dabei zu beobachten, verfälscht die Messung: Sobald
  sich der Debugger anhängt, sieht er keine Anfragen mehr.
- Was gesichert ist: Die Dateien **liegen** im Zwischenspeicher (21 von 21),
  und über `Network.emulateNetworkConditions {offline:true}` liefert der
  Worker sie unter Chromium 141 auch aus (518 Zeichen auf der Startseite).

**Was es voraussetzt.** Einen Chromium in der Fassung, die der Runner
benutzt, auf einer Maschine, auf der man ihn beobachten kann. Erst damit
lässt sich sagen, ob sich der Browser geändert hat oder die App etwas falsch
macht — und ohne diese Trennung ist jede Änderung am Service Worker geraten.

Bis dahin bleibt der Schritt im Job `messungen` rot. Das ist beabsichtigt:
Ein roter Schritt, der die Wirklichkeit zeigt, ist mehr wert als ein grüner,
der sie verfehlt.
