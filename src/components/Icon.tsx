...
export function IconTile({ name, tone = 'gold', size = 40 }: { name: IconName; tone?: 'gold' | 'green' | 'blue' | 'red' | 'violet'; size?: number }) {
  const tones: Record<string, { bg: string; color: string; border: string }> = {
    ...
    green: { bg: 'var(--ok-dim)', color: 'var(--ok-lesbar)', border: 'rgba(88,179,104,0.3)' },
    blue: { bg: 'var(--info-dim)', color: 'var(--info-lesbar)', border: 'rgba(85,144,217,0.3)' },
    red: { bg: 'var(--danger-dim)', color: 'var(--danger-lesbar)', border: 'rgba(224,92,85,0.3)' },
    violet: { bg: 'var(--kategorie-sozial-schwach)', color: 'var(--kategorie-sozial-lesbar)', border: 'rgba(155,127,212,0.3)' },
  };
  const t = tones[tone];
  return (
    <span
